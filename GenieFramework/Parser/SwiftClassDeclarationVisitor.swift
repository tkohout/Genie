...
class SwiftClassDeclarationVisitor: SwiftVisitor<Declaration> {
    ...
    override func visitClass_declaration(_ ctx: SwiftParser.Class_declarationContext) -> Declaration {
        ...
        //TODO: Grammar has second access level modifier?
        let accessLevelModifier = ctx.access_level_modifier().first?.accept(SwiftAccessLevelModifierVisitor())
        ...
        var declarations = body.class_member().mapJoinedByIndentation(parentCtx: body) { member in
